...
namespace SQLInjection
{
    public class CheckSQL
    {
        ...
        public static void Check(HttpApplication application,string keywords)
        {
            ...
            foreach (string keyword in sqlkeyword)
            {
                ...
                if (request.Form!= null)
                {
                    for (int k = 0; k <request.Form.Count; k++)
                    {
                        ...
                        string getip = Find(request, keyword,getsqlkey, formValue, (int)CheckItem.form);
                    }
                }
                // -----------------------防 GET 注入-----------------------
                if (request.QueryString != null)
                {
                    for (int k = 0; k <request.QueryString.Count; k++)
                    {
                        ...
                        string getip = Find(request, keyword, getsqlkey, queryValue, (int)CheckItem.query);
                    }
                }
                // -----------------------防 Cookies 注入-----------------------
                if (request.Cookies != null)
                {
                    for (int k = 0; k < request.Cookies.Count; k++)
                    {
                        ...
                        string getip = Find(request, keyword,getsqlkey, cookieValue, (int)CheckItem.cookie);
                    }
                }
            }
            //watch.Stop();
        ...
        private static string Find(HttpRequest request, string keyword,string key, string value,int type)
        {
            string optionType = "";
            switch (type)
            { 
